...
import {caseStudyOverrides} from '../../../lib/case-study-overrides';
...
export default async function CaseStudy({params}:{params:Promise<{slug:string}>}){
 ...
 const override=caseStudyOverrides[c.slug]||{};
 const proof=override.proof||c.proof;
 const outcome=override.outcome||c.outcome;
 const caveat=override.caveat||c.caveat;
  ...
  <article className={styles.detail}>
   <div className="container">
    <header className={styles.hero}>
     <div className={styles.kicker}><span>{c.category}</span><span className={c.type==='Illustrative strategy'?styles.illustrative:styles.experience}>{c.type==='Illustrative strategy'?'Strategy exercise':'First-hand experience'}</span></div>
     <h1 className="display h-xl">{c.title}</h1>
     <p className={styles.lead}>{c.summary}</p>
     <div className={styles.proofBand}>
      <div><span className={styles.eyebrow}>Proof of work</span><p>{proof}</p></div>
     </div>
    </header>
    <section className={styles.lesson}><div><span className={styles.eyebrow}>The strategic lesson</span><h2>{c.lesson}</h2></div><p>{caveat||'This case is written to show the problem, reasoning, operating choices and measurement logic behind the work. The objective is to make the decisions visible, not turn one outcome into a generic success story.'}</p></section>
    ...
    <section className={styles.outcomeSection}><div><span className={styles.eyebrow}>05 · Outcome</span><h2>What changed?</h2></div><div>{outcome.map(item=><p key={item}>{item}</p>)}</div></section>
