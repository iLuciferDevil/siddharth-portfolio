  ...
  <article className={styles.detail}>
   <div className="container">
    ...
    <section className={styles.lesson}>
     <div><span className={styles.eyebrow}>The strategic lesson</span><h2>{c.lesson}</h2></div>
     <p>{c.caveat||'This case is written to show the problem, reasoning, operating choices and measurement logic behind the work. The objective is not to turn one outcome into a generic success story.'}</p>
    </section>

    <section className={styles.twoCol}>
     <div><span className={styles.eyebrow}>01 · The problem</span><h2>What was actually stuck?</h2></div>
     <div><p>{c.problem}</p></div>
    </section>
    <section className={styles.twoCol}>
     <div><span className={styles.eyebrow}>02 · Diagnosis</span><h2>What did the evidence suggest?</h2></div>
     <div><p>{c.diagnosis}</p></div>
    </section>

    <section className={styles.workSection}>
     <div><span className={styles.eyebrow}>03 · The work</span><h2 className="display h-lg">From diagnosis to intervention.</h2></div>
     <div className={styles.steps}>{c.approach.map((item,i)=><div className={styles.step} key={item}><span>0{i+1}</span><p>{item}</p></div>)}</div>
    </section>

    <section className={styles.measureSection}>
     <div><span className={styles.eyebrow}>04 · Measurement</span><h2>What should move if the strategy is working?</h2></div>
     <div className={styles.measureList}>{c.measurement.map(item=><div key={item}><CheckCircle2 size={18}/><span>{item}</span></div>)}</div>
    </section>

    <section className={styles.outcomeSection}>
     <div><span className={styles.eyebrow}>05 · Outcome</span><h2>What changed?</h2></div>
     <div>{c.outcome.map(item=><p key={item}>{item}</p>)}</div>
    </section>

    <section className={styles.principles}>
     <div><span className={styles.eyebrow}>06 · What I would carry forward</span><h2>Principles, not playbooks.</h2></div>
     <div>{c.principles.map((item,i)=><div className={styles.principle} key={item}><span>0{i+1}</span><p>{item}</p></div>)}</div>
    </section>

    <section className={styles.bottomNav}>
      <div><span className={styles.eyebrow}>More proof of work</span><h2>See how the same thinking changes across different marketing problems.</h2></div>
      <div className={styles.related}>{related.map(r=><Link href={`/case-studies/${r.slug}`} key={r.slug}><span>{r.category}</span><strong>{r.title}</strong><ArrowUpRight size={17}/></Link>)}</div>
    </section>
   </div>
  </article>
