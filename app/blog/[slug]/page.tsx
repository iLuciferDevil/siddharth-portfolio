...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){
  ...
  return {title:p.title,description:p.excerpt,keywords:[p.tag,p.category,'marketing strategy','growth strategy','marketing consultant'],alternates:{canonical:`https://siddharthbhattacharjee.in/blog/${p.slug}`}};
}
...
export default async function PostPage({params}:{params:Promise<{slug:string}>}){
  ...
  const articleSchema={
    ...
    keywords:[p.tag,p.category,'marketing strategy','growth','go-to-market'].join(', ')
  };
  const breadcrumbSchema={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[
    {'@type':'ListItem',position:1,name:'Writing',item:'https://siddharthbhattacharjee.in/blog'},
    {'@type':'ListItem',position:2,name:p.title,item:`https://siddharthbhattacharjee.in/blog/${p.slug}`}
  ]};
